...
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en"> <!--<![endif]-->
    <head>
        ...
        <meta property="place:lat" content="{{ place['display']['address'].get('latitude') }}"/>
        <meta property="place:lng" content="{{ place['display']['address'].get('longitude') }}"/>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <main style="position: absolute; top: 64px; bottom: 64px; left: 0px; right: 0px; z-index: 2;">
            <section id="place" style="margin-bottom: 74px;">
                <div class="container">
                    <div class="row">
                        <div class="col-xs-12">
                            <h1><strong>{{ place['display']['name'] }}</strong></h1>
                            <h2>{{ place['display']['address'].get('city') }}, {{ place['display']['address'].get('state') }}</h2>
                            <h3>{{ place['display']['business_hours'] }}</h3>
                            <h4><a href="{{ place['display']['website'] }}"><i class="fa fa-globe"></i> {{ place['display']['website'] }}</a></h4>
                            <h4><a href="tel:{{ place['display']['phone'] }}"><i class="fa fa-phone"></i> {{ place['display']['phone'] }}</a></h4>

                            <h2><strong>Report:</strong></h2>

                            <div class="row">
                                <button id="dirty-bathroom" type="button" class="btn btn-lg btn-default col-xs-12 col-sm-9"  data-toggle="modal" data-target="#modal-reported"><strong>Dirty Bathroom Stall</strong></button>
                                <button id="dirty-bathroom-urgent" type="button" class="btn btn-lg btn-warning col-xs-12 col-sm-3" data-toggle="modal" data-target="#modal-reported"><strong>Urgent!</strong></button>
                            </div>

                            <div class="row">
                                <button id="not-open" type="button" class="btn btn-lg btn-default col-xs-12 col-sm-12"  data-toggle="modal" data-target="#modal-reported"><strong>Not Open During Store Hours</strong></button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                }).setView([$('meta[property="place:lat"]').attr("content"), $('meta[property="place:lng"]').attr("content")], 16);;

                var marker = L.marker([$('meta[property="place:lat"]').attr("content"), $('meta[property="place:lng"]').attr("content")]).addTo(map);
